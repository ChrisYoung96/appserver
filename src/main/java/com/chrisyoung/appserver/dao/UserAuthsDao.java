...
public interface UserAuthsDao {
    ...
    //查找验证条目（如：根据手机和密码查找）
    @Select("select count(*) from user_auths where identify=#{identify} and credential=#{credential}")
    int findAuth(@Param("identify") String identify, @Param("credential") String credential);
    ...
    //修改用户的某一验证条目
    @Update("update user_auths set credential=#{credential},refreshtoken=#{refreshtoken} where u_id=#{uId} and identify=#{identify}")
    int updateAuth(UserAuths newAuth);
}
