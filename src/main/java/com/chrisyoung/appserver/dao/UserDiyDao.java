...
public interface UserDiyDao {
    @Insert("insert into user_diy (d_id,u_id,d_type,d_kind,d_version,delflag) values(#{dId},#{uId},#{dType},#{dKind},#{dVersion},#{delflag})")
    int addNewKind(UserDiy newkind);

    @Delete("delete from user_diy where d_id=#{dId}")
    int deleteKind(@Param("dId") String dId);

    @Select("select * from user_diy where u_id=#{uId} and delflag=0")
    @Results({
    ...
    @Update("update user_diy set u_id=#{uId},d_type=#{dType},d_kind=#{dKind},d_version=#{dVersion},delflag=#{delflag} where d_id=#{dId}")
